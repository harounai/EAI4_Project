...
GESTURE_MODEL  := $(ARTIFACT_DIR)/gesture_model.tflite
ACCESSORY_MODEL := $(ARTIFACT_DIR)/accessory_model.tflite
...
DATASET_CAPTURE_BINARY := $(BUILD_DIR)/dataset_capture
...
$(APP_BINARY): CMakeLists.txt toolchains/aarch64.cmake $(CPP_SOURCES) $(TFLITE_READY) | $(BUILD_DIR_STAMP)
	...
	cmake --build "$(BUILD_DIR)" --parallel "$(BUILD_JOBS)" --target "$(APP_NAME)" --target "dataset_capture"

build: $(APP_BINARY)

deploy: $(APP_BINARY) $(DATASET_CAPTURE_BINARY)
	bash scripts/deploy_to_pi.sh

run: $(APP_BINARY) $(GESTURE_MODEL) $(ACCESSORY_MODEL)
