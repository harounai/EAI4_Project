...
def load_dataset(data_dir: Path, seed: int):
    ...
    if found_classes != CLASS_NAMES:
        raise ValueError(
            ...
            f"  Got:      {found_classes}\n"
            f"  Fix: rename folders to exactly: absent/  present/"
        )
...
def make_model(num_classes: int) -> tf.keras.Model:
    ...
    model = tf.keras.Model(inputs, outputs, name="accessory_mobilenetv2")
    model.compile(
